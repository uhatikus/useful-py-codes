...
(async () => {
  try {
    ...
    if (userQueryRes.ok) {
  // The request was successful, proceed with parsing the JSON.
  const userQueryJson = await userQueryRes.json();
  console.log(userQueryJson);
} else {
  // The request was not successful. Handle the error.
  const errorStatus = userQueryRes.status;
  const errorText = userQueryRes.statusText;

  console.error(`Error: ${errorStatus} - ${errorText}`);

  // Optionally, you can try to get more details from the response body.
  try {
    const errorBody = await userQueryRes.json();
    console.error('Error Body:', errorBody);
  } catch (error) {
    console.error('Error parsing error body as JSON:', error);
  }
}
